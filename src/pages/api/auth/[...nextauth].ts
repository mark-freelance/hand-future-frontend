...
import { EMAIL_FROM, EMAIL_SERVER } from "@/lib/env"
...
export const authOptions: NextAuthOptions = {
  providers: [
    EmailProvider({
      server: EMAIL_SERVER,
      from: EMAIL_FROM, // maxAge: 24 * 60 * 60, // How long email links are valid for (default 24h)
      ...
      async sendVerificationRequest(params) {
        ...
        const result = await transport.sendMail({
          to: identifier,
          from: provider.from,
          subject: `Your Activation Code：${token}`, // text: text({ url, host }),
