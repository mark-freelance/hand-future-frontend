...
export const CardPage = () => {
  ...
  const [fontWeightName, setFontWeightName] = useState<FONT_WEIGHT>(
    FONT_WEIGHTS[6],
  );
  const [fontWeightTitle, setFontWeightTitle] = useState<FONT_WEIGHT>(
    FONT_WEIGHTS[8],
  );
  const [fontWeightContent, setFontWeightContent] = useState<FONT_WEIGHT>(
    FONT_WEIGHTS[3],
  );
  const [qrCodeUrl, setQrCodeUrl] = useState(
    "https://gkleifeng.notion.site/da7ad92cb3414e6891c80e52541a6678",
  );
      ...
      <div className="p-4 m-auto flex flex-wrap gap-8 text-white">
        ...
        <div className={"flex flex-col gap-1"}>
          ...
          <label htmlFor="hero-avatar" className="flex items-center">
            ...
            {/* 在iphone浏览器里不显示 */}
            <BaseAvatar
              url={normalizeImageUri(data.avatar)}
              text={data.name ?? ""}
            />
          </label>
          ...
          <InputText
            type="articleTitle"
            placeholder={data.articleTitle}
            update={update}
          />
          <InputTextArea
            rows={3}
            type="articleContent"
            placeholder={data.articleContent}
            update={update}
          />
          ...
          <div className="form-control w-full">
            <label htmlFor="select-name-weight" className="input-group">
              ...
              <select
                ...
                onChange={(e) =>
                  setFontWeightName(e.target.value as unknown as FONT_WEIGHT)
                }
              >
          ...
          <div className="form-control w-full">
            <label htmlFor="select-title=font-weight" className="input-group">
              ...
              <select
                id="select-title=font-weight"
                className="select select-bordered flex-grow"
                defaultValue={fontWeightTitle}
                onChange={(e) =>
                  setFontWeightTitle(e.target.value as unknown as FONT_WEIGHT)
                }
              >
                {FONT_WEIGHTS.map((weight) => (
                  <option key={weight} value={weight}>
          ...
          <div className="form-control w-full">
            <label htmlFor="select-content-weight" className="input-group">
              ...
              <select
                id="select-content-weight"
                className="select select-bordered flex-grow"
                defaultValue={fontWeightContent}
                onChange={(e) =>
                  setFontWeightContent(e.target.value as unknown as FONT_WEIGHT)
                }
              >
                {FONT_WEIGHTS.map((weight) => (
                  <option key={weight} value={weight}>
