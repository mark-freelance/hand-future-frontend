...
export const user2hero = (user: IUserInNotion): IHeroWithoutUser => ({
    id: user.id, // title: user.properties.title.rich_text[0]?.plain_text,
...
export const persistHeroAvatar = async (user: IHeroWithoutUser,): Promise<IHeroWithoutUser> => {

    if (user.avatarOrigin) {

        const res = await fileRemote2Local(user.avatarOrigin);
...
export const user2heroRelation = (user: IUserInNotion,): {
    fromId: string; toId: string
}[] => user.properties.携手嘉宾.relation.map((r) => ({
    fromId: user.id, toId: r.id,
}));
