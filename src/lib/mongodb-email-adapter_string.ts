...
export const format = {
	...
	to<T = Record<string, unknown>>(object: Record<string, any>) {
		let { id } = object
		if (!id) {
			id = object.id // force id to exist
		}
		const newObject: Record<string, unknown> = {
			_id: id,
			id: id,
 ...
 * ```
 **/
export function MongodbEmailAdapter(
	...
	return {
		...
		async getUser(id) {
			const user = await (await db).U.findOne({ _id: id })
			if (!user) return null
			return from<AdapterUser>(user)
		},
