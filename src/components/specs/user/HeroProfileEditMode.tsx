...
export const HeroProfileEditMode = ({
  ...
  const [changed, setChanged] = useState<Partial<IUserWithId>>({});
    ...
    <div className="w-full grow flex flex-col gap-2">
      ...
      <div className="shadow-blackA7 w-full max-auto overflow-hidden rounded-md shadow-[0_2px_10px] relative">
        <AspectRatio.Root ratio={16 / 5}>
          <div
            style={{
              backgroundImage: `linear-gradient(to right, rgba(9, 50, 50, 0), rgba(9, 148, 143, 1)), url('${
                normalizeImageUri(changed.cover || user.cover || undefined) ||
                BG_COVER_FALLBACK
              }')`,
          ...
          <Button>
            <Label className="absolute bottom-2 right-2" htmlFor={"cover"}>
              更改封面
            </Label>
            <MyImageUploader
              hidden
        ...
        <div className="absolute p-12 left-0 bottom-0 max-w-screen-sm flex flex-col gap-2">
          <div className="flex items-center gap-4">
            <Label id={"avatar"}>
              <BaseAvatar
                url={normalizeImageUri(
                  changed.avatar || user.avatar || undefined,
                )}
                text={user.name}
              />
              <MyImageUploader
                hidden
                id={"cover"}
                onUploaded={bindChange("avatar")}
              />
            </Label>
            <Input
              placeholder={"你怎么还没有名字儿呀，快起一个吧！"}
      ...
      {works.length ? (
        <div className="gap-4 grid md:grid-cols-2">
          {works.map((work) => (
            <WorkPresentation key={work.id} work={work} isEditable />
          ))}
        </div>
      ) : (
